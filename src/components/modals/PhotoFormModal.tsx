...
const photoSchema = yup.object({
  ...
  url: yup.string().url("آدرس نامعتبر").required("وارد کردن آدرس عکس الزامی است"),
  thumbnailUrl: yup.string().url("آدرس نامعتبر").required("وارد کردن آدرس بند انگشتی الزامی است"),
  albumId: yup.number().min(1, "شناسه آلبوم معتبر نیست").required("شناسه آلبوم الزامی است"),
});
...
const PhotoFormModal = ({ photoId, onClose }: PhotoFormModalProps) => {
  ...
  const {
    ...
    values: photo
      ? { title: photo.title, url: photo.url, thumbnailUrl: photo.thumbnailUrl, albumId: photo.albumId }
      : undefined,
    defaultValues: {
      title: "",
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-0 md:p-4" dir="rtl">
      ...
      <div className="bg-white rounded-none md:rounded-lg shadow-xl w-full h-full md:h-auto md:max-w-lg md:p-6 p-4 relative z-10 overflow-y-auto">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">عنوان عکس</label>
            <input
              type="text"
              {...register("title")}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">آدرس عکس</label>
            <input
              type="text"
              {...register("url")}
              className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">آدرس بند انگشتی</label>
            <input
              type="text"
...
p-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">شناسه آلبوم</label>
            <input
              type="number"
              {...register("albumId")}
              className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
